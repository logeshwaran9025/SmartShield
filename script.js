...
const chartDefaults = {
  ...
  scales:{ x:{ grid:{ color:"rgba(0,0,0,0.04)" }, ticks:{ color:"#94a3b8", font:{ size:11 } } }, y:{ grid:{ color:"rgba(0,0,0,0.04)" }, ticks:{ color:"#94a3b8", font:{ size:11 } } } }
};
...
function initAdminCharts() {
  ...
  const ctx2 = document.getElementById("disruptionPieChart");
  if (ctx2 && !state.chartsInitialized.pie) {
    ...
    new Chart(ctx2, {
      ...
      options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:true, position:"right", labels:{ color:"#475569", font:{ size:11 }, padding:12, boxWidth:12 } } } }
    });
  }


  const ctx3 = document.getElementById("predictiveChart");
...
function initTrendCharts() {
  ...
  const ctx2 = document.getElementById("planPieChart");
